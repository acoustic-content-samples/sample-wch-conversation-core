...
module.exports = function setup(options, imports, register) {
  ...
  if (options.enabled) {
    let {defaultLanguage, supportedLanguages, serviceName = 'wch-languagetranslator'} = options;
    ...
    identifyLanguage = function (message, conversationPayload) {
      ...
      return new Promise((resolve, reject) => {
        if (locale && context && context.setoutputlang) {
          ...
          translationService.identify(
            ...
            (err, language) => {
              if (err) {
                ...
                resolve({outputlang: defaultLanguage, setoutputlang: false});
              }
            }
          );
        }
          ...
          let userLocale = locale.split('-')[0];
          let newLocale = options.supportedLanguages.includes(userLocale) ? userLocale : options.defaultLanguage;
          logger.debug('No context yet! But a locale is given: %s', newLocale);
          resolve({outputlang: newLocale, setoutputlang: false});
        }
        else {
      ...
      .then(value => logger.methodExit('identifyLanguage', value));
    };
  }
  else {
