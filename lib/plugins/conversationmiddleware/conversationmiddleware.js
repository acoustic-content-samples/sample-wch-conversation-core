    ...
    .forEach(middlewareConfig => {
      ...
      middleware.after = function(message, conversationResponse, callback) {
        ...
        Promise.all(middlewareAfter.map(middleware => middleware.after(message, conversationResponse)))
          .then(results => {
            let _conversationResponse = conversationResponse;
            results.forEach(result => {
              _conversationResponse.context = Object.assign({}, _conversationResponse.context, result);
            });
            middleware.updateContextAsync(message.user, _conversationResponse.context)
              .then(() => {
                callback(null, _conversationResponse);
              });
          })
          .catch(err => {
