...
const debug = require('debug')('conversation-core:wchconversationV1');
 ...
 */
const processLocationResponse = function (watsonData, templating, wchDelivery, wchHostUrl) {
  return new Promise((resolve, reject) => {
    ...
    .catch(reject)
    .then(searchRes => processAttachments(searchRes, wchDelivery, wchHostUrl))
    .catch(reject)
 ...
 * @return {Promise}           When the promise resolves the enriched search result from WCH is returned.
 */
const processConversationResponse = function (watsonData, templating, wchDelivery,wchHostUrl) {
  return new Promise((resolve, reject) => {
    let { context, output, intents, entities, context: {nodename}, output: {required = []} } = watsonData;
 ...
 * @return {Promise}           When the promise resolves the enriched search result from WCH is returned.
 */
const processFollowUpResponse = function (watsonData, templating, wchDelivery,wchHostUrl) {
  return new Promise((resolve, reject) => {
...
const processAttachment = function (attachment, wchDelivery, wchHostUrl) {
  return new Promise((resolve, reject) => {
    wchDelivery.send({
      baseUrl: wchHostUrl,
      uri: attachment.url
...
const processAttachments = function (searchResult, wchDelivery, wchHostUrl) {
  return new Promise((resolve, reject) => {
    let { numFound, documents } = searchResult;
    if ( numFound > 0
      ...
      if (attachments.length > 0) {
        let attachmentsPromises = attachments.map(value => processAttachment(value, wchDelivery, wchHostUrl));
...
class WchConversationV1 {
  constructor(templating, wchDelivery, wchHostUrl, {enabled, ttl}) {
    debug('WCH Response Caching Enabled: %b', enabled);
      ...
      generateMD5Key(watsonData)
        .then(hashKey => {
          ...
          if (cachedResultSet) {
            ...
            if (conversationResp && conversationResp.searchResult && conversationResp.searchResult.documents) {
              arrayutils.shuffle(conversationResp.searchResult.documents);
            }
            if (locationResp && locationResp.searchResult && locationResp.searchResult.documents) {
              arrayutils.shuffle(locationResp.searchResult.documents);
            }
            if (followupResp && followupResp.searchResult && followupResp.searchResult.documents) {
            ...
            Promise.all([
              processLocationResponse(watsonData, this.templating, this.wchDelivery, this.wchHostUrl),
              processConversationResponse(watsonData, this.templating, this.wchDelivery, this.wchHostUrl),
              processFollowUpResponse(watsonData, this.templating, this.wchDelivery, this.wchHostUrl)
            ])
