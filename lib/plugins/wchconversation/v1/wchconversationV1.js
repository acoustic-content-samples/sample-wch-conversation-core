...
const processConversationResponse = function (watsonData, templating, wchDelivery, wchHostUrl, logger, defaultLocale) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let requiredEntities = required.reduce((resarry, reqEntitiy) => {
      const entityValues = entities.filter(ele => ele.entity === reqEntitiy);
      let reqQryEntities = [];
      if (entityValues.length === 0) {
        // TODO: Check if typeof context[reqEntitiy] === 'String'
        // Then create the array manually
        reqQryEntities = context[reqEntitiy] ? context[reqEntitiy].map(entity => (`+("${entity.value}"^20 OR "${reqEntitiy}")`)) : [];
      }
      else {
        reqQryEntities = entityValues.map(entity => (`+"${entity.value}"`));
      }

      logger.debug('Required entities for entitiy %s %o', reqEntitiy, reqQryEntities);
      ...
      return (entityValues) ? resarry.concat(reqQryEntities) : resarry;
    }, []);
    ...
    const operator = (requiredEntities.length > 0) ? 'AND' : 'OR';
    const queryEntities = requiredEntities.concat(optionalEntities);
