 ...
 */
const processLocationResponse = function (watsonData, templating, wchDelivery, wchHostUrl, logger) {
  return new Promise((resolve, reject) => {
 ...
 * @return {Promise}           When the promise resolves the enriched search result from WCH is returned.
 */
const processConversationResponse = function (watsonData, templating, wchDelivery, wchHostUrl, logger) {
 ...
 * @return {Promise}           When the promise resolves the enriched search result from WCH is returned.
 */
const processFollowUpResponse = function (watsonData, templating, wchDelivery, wchHostUrl, logger) {
...
class WchConversationV1 {
  constructor(templating, wchDelivery, wchHostUrl, {enabled, ttl, logging}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      generateMD5Key(watsonData)
        ...
        .then(({hashKey, cachedResultSet}) => {
          if (!cachedResultSet) {
            return Promise.all([
              processLocationResponse(watsonData, this.templating, this.wchDelivery, this.wchHostUrl, this.logger),
              processConversationResponse(watsonData, this.templating, this.wchDelivery, this.wchHostUrl,this.logger),
              processFollowUpResponse(watsonData, this.templating, this.wchDelivery, this.wchHostUrl, this.logger)
            ])
        ...
        .then(value => {
          this.logger.debug('WchConversationTime in ms: ', (Date.now()-startTime));
