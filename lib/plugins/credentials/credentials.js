...
const {join, parse, resolve, isAbsolute} = require('path');
...
module.exports = function setup(options, imports, register) {
  ...
  let pathToPrivKey = options.pathPrivKey || join(homedir(), '.ssh', 'id_rsa'); // Private Key Contains the Public Key

  if (!isAbsolute(pathToPrivKey)) {
    logger.debug("The given path %s is not absolute. Resolving", pathToPrivKey);
    pathToPrivKey = untildify(pathToPrivKey);
  }

  const config = {
    pubKPath: pathToPrivKey,
    privKPath: pathToPrivKey,
  ...
  if (!config.readOnly) {
    ...
    /**
     * Only availabe if the credentials are modifiable. Makes changes to the config.
     * @param  {String} options.pubKPath  Path to public key. Normally contained in the private key.
     * @param  {String} options.privKPath Path to the private key.
     * @param  {Boolean} options.encrypted Identification if the credentials file is locally encrypted.
     * @param  {String} options.defaultCredPath Path to the credentials
     */
    _updateConfig = function (updatedConfig) {
