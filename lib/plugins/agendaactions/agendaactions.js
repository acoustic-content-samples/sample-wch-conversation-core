      ...
      try {
        ...
        if (actionType === 'client' && actionName === 'GetSessionContent') {
          ...
          if (actionParams.date && actionParams.date.length > 0 && actionParams.time && actionParams.time.length > 0) {
            utcDate = new Date(`${actionParams.date[0].value} ${actionParams.time[0].value}`);
          }
          else if (actionParams.time && actionParams.time.length > 0) {
            utcDate = new Date(`2018-06-05 ${actionParams.time[0].value}`);
          }
          else {
            const date = new Date();
            date.setDay(6);
            date.setMonth(6);
            utcDate = new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate(), date.getHours(), date.getMinutes(), date.getSeconds()));
          }

          logger.debug("Date %s", utcDate.toISOString());
          let queryParams = {
