      ...
      try {
        ...
        if (actionType === 'client' && actionName === 'GetSessionContent') {
          ...
          if (actionParams.date && actionParams.date.length > 0 && actionParams.time && actionParams.time.length > 0) {
            const dateDe = new Date(`${actionParams.date[0].value}T${actionParams.time[0].value}`).toLocaleString('en-US', {timezone: 'Europe/Berlin'});
            utcDate = new Date(dateDe);
          }
          else if (actionParams.time && actionParams.time.length > 0) {
            const dateDe = new Date(`2018-06-05T${actionParams.time[0].value}`).toLocaleString('en-US', {timezone: 'Europe/Berlin'});
            utcDate = new Date(dateDe);
          }
          else {
